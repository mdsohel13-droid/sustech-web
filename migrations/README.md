...
> Loader note: the bundled `payload` CLI fails to load the TS config on this
> project's Node versions (jiti/tsx → `ERR_METHOD_NOT_IMPLEMENTED`), so
> `pnpm migrate` and `pnpm migrate:status` run via small jiti scripts in
> `cms/scripts/` (the same loader `generate:types` uses) — these work on the VPS
> and locally. `migrate:create` (generating a new migration) still shells out to
> the Payload CLI and is best run where that works, or generated by an agent and
> committed; either way **review the SQL before committing**.
