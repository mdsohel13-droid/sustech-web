...
test.describe("Sector (solutions) detail pages", () => {
  test("a seeded sector renders with one h1, challenges and capabilities", async ({ page }) => {
    const res = await page.goto("/solutions/manufacturing-rmg-textile");
    expect(res?.status()).toBe(200);
    const h1 = page.getByRole("heading", { level: 1 });
    await expect(h1).toHaveCount(1);
    await expect(h1).toHaveText("Manufacturing & RMG / Textile");
    ...
    await expect(
      page.locator('a[href="/projects?sector=manufacturing-rmg-textile"]').first(),
    ).toBeVisible();
  });
